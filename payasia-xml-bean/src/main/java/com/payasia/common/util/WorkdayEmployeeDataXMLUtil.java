...
public class WorkdayEmployeeDataXMLUtil {
	
	private static String NAMESPACE_URI = "http://payasia.mind.com/xml/bean/workday/empdata";
	...
	private static Schema getDocumentSchema() throws SAXException {
		...
		Schema schema = sf.newSchema(Thread.currentThread()
				.getContextClassLoader().getResource("Workday-EMF_Data.xsd"));
		return schema;
	}
	...
	public static PayrollExtractEmployees getXMLData(byte[] fileByteArray) {
		...
		try {
			...
			/*xsr = new StreamReaderDelegate() {
		        @Override
		        public String getNamespaceURI() {
		        	return "http://payasia.mind.com/xml/bean/workday/empdata";
		        }
			};*/
	...
	public static String convertEmployeeToXmlString(EmployeeType employeeObj) {
		...
		try {
			...
			Marshaller marshaller = WorkdayEmployeeDataXMLUtil.getDocumentMarshaller();
	...
	public static JAXBElement<EmployeeType> convertXmlStringToEmployee(String xmlString) {
		...
		try {
			Unmarshaller unmarshaller = WorkdayEmployeeDataXMLUtil.getEmployeeUnmarshaller();
			XMLInputFactory xif = XMLInputFactory.newInstance();
