...
@Component
public class LionTimesheetReviewerLogicImpl implements
		LionTimesheetReviewerLogic {

	@Resource
	EmployeeTimesheetReviewerDAO lundinEmpReviewerDAO;

	@Override
	public List<LundinEmployeeTimesheetReviewerDTO> getWorkFlowRuleList(
	...
	@Override
	public LundinTimesheetReviewerForm getLionWorkFlow(Long companyId) {
		int otRuleVal = 0;
	...
	@Override
	public LundinReviewerResponseForm getLionReviewers(String searchCondition,
		...
		if (searchCondition.equals(PayAsiaConstants.EMAIL)) {
			if (StringUtils.isNotBlank(searchText)) {
				...
				try {
					conditionDTO.setEmail(URLDecoder
							.decode(searchText, "UTF-8"));
				} catch (UnsupportedEncodingException ex) {
					LOGGER.error(ex.getMessage(), ex);
				}
			}
		...
		for (LundinOTReviewerDetailView otReviewerVO : otReviewerListVO) {
			...
			otReviewerForm.setEmail(otReviewerVO.getEmail());
			/*ID ENCRYPT */
			otReviewerForm.setEmployeeId(FormatPreserveCryptoUtil.encrypt(otReviewerVO
					.getLundinOTReviewerDetailViewPK().getEmployeeId()));
	...
	@Override
	public LundinReviewerResponseForm getLionReviewerData(Long employeeId,
			Long companyId) {
		...
		for (EmployeeTimesheetReviewer employeeOTReviewerVO : employeeOTReviewerListVO) {
			...
			otReviewerForm.setEmployeeId(employeeOTReviewerVO.getEmployee()
					.getEmployeeId());
			if (StringUtils.isNotBlank(employeeOTReviewerVO.getReviewerEmail())) {
				otReviewerForm
						.setEmail(employeeOTReviewerVO.getReviewerEmail());
			}
			...
			otReviewerForm.setLundinReviewerId(employeeOTReviewerVO
		...
		List<TimesheetWorkflow> workFlowRuleMasterList = lundinWorkflowDAO
				.findByCompanyId(companyId);
		// WorkFlowRuleMaster workFlowRuleMasterVO =
		// workFlowRuleMasterDAO.findByCondition(leaveSchemeId, ruleName)
		// .findByClaimTemplateCondition(claimTemplateId,
	...
	@Override
	public LionTimesheetReviewerForm checkLionReviewer(Long employeeId,
			Long companyId) {
		LionTimesheetReviewerForm otReviewerForm = new LionTimesheetReviewerForm();
	...
	@Override
	public void saveLionReviewer(
			LionTimesheetReviewerForm otTimesheetReviewerForm, Long companyId) {
		...
		try {
			...
			for (int reviewerCount = 1; reviewerCount <= ruleValue; reviewerCount++) {
				...
				employeeOTReviewer.setReviewerEmail(otTimesheetReviewerForm
						.getEmail());
	...
	@Override
	public void deleteLionReviewer(Long employeeId) {
...
/*IDENCRYPT*/
	...
	@Override
	public void updateLionReviewer(
			...
			for (int reviewerCount = 1; reviewerCount <= ruleValue; reviewerCount++) {
				...
				if (employeeOTReviewerId != null) {
					...
					if (employeeReviewerId != null) {
						employee2 = employeeDAO.findById(employeeReviewerId);
						employeeOTReviewer
								.setReviewerEmail(otTimesheetReviewerForm
										.getEmail());
						employeeOTReviewer.setEmployeeReviewer(employee2);
	...
	@Override
	public String configureLionRevWorkflow(Long companyId,
		...
		if (otTimesheetReviewerForm.getWorkFlowLevel().toUpperCase()
				...
				|| otTimesheetReviewerForm.getWorkFlowLevel().toUpperCase()
						.equals("1")) {
	...
	@Override
	public LundinTimesheetReviewerForm getLionWorkFlowLevel(Long companyId) {
		LundinTimesheetReviewerForm otReviewerForm = new LundinTimesheetReviewerForm();
	...
	@Override
	public LundinTimesheetReviewerForm importLionReviewer(
			...
			try {
				if (!key.equalsIgnoreCase(PayAsiaConstants.COLUMN_NAME_EMPLOYEE_NUMBER)) {
					...
					for (int otReviewerCount = 1; otReviewerCount <= otRuleVal; otReviewerCount++) {
						...
						employeeOTReviewer.setReviewerEmail(hrisReviewersDTO
								.getReviewer1Email());
		...
		for (HashMap<String, String> map : otReviewerExcelFieldForm
			...
			for (Iterator<String> iterator = keySet.iterator(); iterator
				...
				if (key != PayAsiaConstants.HASH_KEY_ROW_NUMBER) {
					...
					if (key.equals(PayAsiaConstants.IMPORT_REVIEWER1_EMAIL)) {
						otReviewersDTO.setReviewer1Email(value);
					}
				...
				if (StringUtils.isNotBlank(otReviewersDTO.getEmployeeNumber())) {
					...
					if (employeeVO != null) {
						if (otRuleVal == 1 || otRuleVal > 1) {
							...
							if (StringUtils.isBlank(otReviewersDTO
									.getReviewer1Email())) {
								DataImportLogDTO dataImportLogDTO = new DataImportLogDTO();
								dataImportLogDTO
										.setColName(PayAsiaConstants.IMPORT_REVIEWER1_EMAIL);
								dataImportLogDTO
										.setRemarks("payasia.lionhk.reviewer.email.empty.alert");
								dataImportLogDTO.setRowNumber(Long
										.parseLong(key));
								dataImportLogDTOs.add(dataImportLogDTO);
